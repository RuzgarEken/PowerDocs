...
| Asset | Use case |
| --- | --- |
| `BSPrecontribution_ModifierCountLimit_Int` | `AttackPower` accepts at most three positive Modifiers, so buffs cap out — while any number of debuffs can still stack on it. |
| `BSPrecontribution_ModifierCountLimit_Float` | The same cap on a Float Stat such as `CritRate`. |
