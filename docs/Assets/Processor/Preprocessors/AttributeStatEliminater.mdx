...
Ships as `AS` and `MAS` only — there is no `BS` form, because the Processor's entire job is to remove Attribute Stats. See the [Processor naming scheme](/docs/Assets/Processor/ProcessorVariations).

| Asset | Use case |
| --- | --- |
| `ASEffectPreprocess_AttributeStatEliminater` | The Effect carries a single `FireAttack` Attribute Stat. A *Fireproof Cloak* eliminates it, and the Effect continues as a plain Base Stat Effect with none of the fire scaling. |
| `MASEffectPreprocess_AttributeStatEliminater` | The Effect carries `PhysicalAttack` **and** `FireAttack`. The cloak eliminates only `FireAttack`; the physical half still contributes normally. |
