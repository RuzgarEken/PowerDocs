...
Ships as `BS`, `AS` and `MAS`, with no container variations — the limit is always a plain Int Stat. See the [Processor naming scheme](/docs/Assets/Processor/ProcessorVariations).

| Asset | Counts against | Use case |
| --- | --- | --- |
| `BSEffectPreprocess_ModifierCountLimit` | The **Target Stat's** Modifiers | `Stun` accepts one Modifier at a time, so a crowd of enemies cannot chain-stun by stacking. |
| `ASEffectPreprocess_ModifierCountLimit` | The Effect's **Attribute Stat** | Only three `PoisonAttack` Modifiers may be active on a target at once, regardless of which Stat they landed on. |
| `MASEffectPreprocess_ModifierCountLimit` | **Every** Attribute Stat | The Effect is blocked as soon as any one of its Attribute Stats has hit its limit. |
