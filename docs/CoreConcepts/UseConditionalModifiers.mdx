...
```mermaid
flowchart LR
    Target(["Effect's Target Stat"]) --> CondProc("ASContribution_ConditionalModifier")
    Queried(["Queried Stat<br/><small>e.g. CritRate, HitRate</small>"]) --> CondProc
    CondProc --> Contribution("Contribution<br/><small>Rate / Flat / Multiplier</small>")
    Contribution --> Result("Adjusted Value<br/><small>used for this execution only</small>")
    ...
    class Target,Queried terminal;
    class CondProc,Contribution,Result stage;
```
